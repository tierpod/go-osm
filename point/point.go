...
func (p ZXY) String() string {
	return fmt.Sprintf("{Zoom:%v X:%v Y:%v}", p.Zoom, p.X, p.Y)
}

// ToLatLong converts ZXY point to latitude and longtitude coordinates.
func (p ZXY) ToLatLong() LatLong {
	var lat, long float64
	n := math.Pi - 2.0*math.Pi*float64(p.Y)/math.Exp2(float64(p.Zoom))
	lat = 180.0 / math.Pi * math.Atan(0.5*(math.Exp(n)-math.Exp(-n)))
	long = float64(p.X)/math.Exp2(float64(p.Zoom))*360.0 - 180.0
